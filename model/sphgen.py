...
class SphGen(nn.Module):
    def __init__(self, cutoff, num_node_types, num_layers, 
        ...
        num_spherical, num_radial, num_flow_layers, train_weight, deq_coeff=0.9, use_gpu=True, n_att_heads=4):
        ...
        self.feat_net = SphereNet(cutoff, num_node_types, num_layers, 
            hidden_channels, int_emb_size, basis_emb_size, out_emb_channels,
        ...
        #self.valence_max = {0: 1, 1: 4, 2: 3, 3: 2, 4: 1}
    ...
    def forward(self, data_batch):
        ...
        focus_ce_func = torch.nn.BCELoss()
        cannot_focus = data_batch['cannot_focus']
        focus_ce = focus_ce_func(focus_score, cannot_focus)
        
        total_loss = ll_node * self.train_weight['node']\
                      + ll_dist * self.train_weight['dist']\
                      + ll_angle * self.train_weight['angle']\
                      + ll_torsion * self.train_weight['torsion']\
                      + focus_ce * self.train_weight['focus']
        

        return total_loss, ll_node, focus_ce, ll_dist, ll_angle, ll_torsion


    def generate(self, type_to_atomic_number, num_gen=100, temperature=[1.0, 1.0, 1.0, 1.0], min_atoms=2, max_atoms=35, focus_th=0.5, add_final=False):
        with torch.no_grad():
            if self.use_gpu:
            ...
            for i in range(max_atoms):
                ...
                if i == 0:
                    ...
                    # print(z.view(-1), pos.view(-1,3), batch.view(-1))
                ...
                # print(focus_node_id.shape, node_feat.shape)
                # print(focus_node_id)
                ...
                # print(focus_node_id.shape, node_emb.shape)
                # print(focus_node_id)
